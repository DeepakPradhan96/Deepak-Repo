...
def step_impl(context):
    assert context.driver.current_url == "https://wowdare.xyz/en/question?utm_site_source=start&utm_site_medium=createButton&utm_site_campaign=user", "url is Wrong"
    time.sleep(2)


@given(u'Validate onesignal pop-up With click on Later Button')
def step_impl(context):
...
def step_impl(context):
    assert context.driver.current_url=="https://wowdare.xyz/en/share?utm_site_source=question&utm_site_medium=saveButton&utm_site_campaign=user"
    print("User Navigate To Share Page Successfully")
...
@given(u'User Friend Validate onesignal pop-up With click on Later Button')
def step_impl(context):
    wait = WebDriverWait(context.driver2, 20)
...
def step_impl(context):
    ...
    context.driver2.find_element_by_xpath("//a[normalize-space()='Confirm']").click()
    time.sleep(0)
...
def step_impl(context):
    context.driver2.execute_script("window.scrollTo(0, 700)")
    context.driver2.find_element_by_xpath("//a[text()='Confirm']").click()
    time.sleep(1)
...
def step_impl(context):
    context.driver2.implicitly_wait(10)
    context.driver2.find_element_by_xpath("//div[@data-file-id='160']").click()

@when(u'User Friend Click On Q3')
def step_impl(context):
    context.driver2.find_element_by_xpath("//div[@value='Green']").click()
...
def step_impl(context):
    context.driver2.find_element_by_xpath("//div[@value='Beautiful Life Partner']").click()
...
def step_impl(context):
    context.driver2.find_element_by_xpath("//div[@data-file-id='188']").click()
...
def step_impl(context):
    context.driver2.find_element_by_xpath("//div[@data-file-id='138']").click()
        
@when(u'User Friend Click On Q14')
def step_impl(context):
    context.driver2.find_element_by_xpath("//div[@value='Two']").click()
